    ...
    <div class="d-flex justify-content-center align-items-center min-h-100 bg-light p-3 p-sm-5">
        ...
        <div class="card shadow-lg px-4 py-1 p-md-5 border-0 login-card">
            <div class="card-body">
                ...
                <!-- Login Form -->
                <form class="d-flex align-items-center flex-column" @submit.prevent="submitLogin">
                    ...
                    <button type="submit" class="btn btn-orng btn-lg fw-bold w-100" :disabled="loading">
                        <span v-if="loading" class="spinner-border spinner-border-sm me-2" role="status"
                            aria-hidden="true"></span>{{ loading ? "Signing In.." : "Login" }}</button>
                </form>

                <!-- Registration Link -->
                <div class="text-center mt-4">
                    <router-link to="/signup" class="txt-orng small fw-medium">
...
export default {
    ...
    methods: {
        async submitLogin() {
            ...
            this.loading = true;
            try {
                const res = await this.userStore.loginWithCredentials("/auth/login", {
                    email: this.email,
...
.txt-orng {
    ...
    /* A bright, vibrant orange for theme */
}
